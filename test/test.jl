...
@test est[1].α≈1.9527275186009831 atol=tolerance
@test est[1].θ == 7.0
@test est[2]≈0.008252950457938835 atol=tolerance

est = estimate_xmin(moby_data, dis_powerlaw, xmins = [2,3,4,10,20])
@test est[1].α≈1.9550379794745847 atol=tolerance
@test est[1].θ == 10.0
@test est[2]≈0.011867106465087929 atol=tolerance

est1 = estimate_xmin(cities, dis_powerlaw)
@test est1[1].α≈1.614392642791718 atol=tolerance
@test est1[1].θ == 1021.0
@test est1[2]≈0.06088582981503132 atol=tolerance

println("test: discrete powerlaw - bootstrap")
est1 = estimate_xmin(electrical_blackout, dis_powerlaw)
@test est1[1].α≈1.2201523540878356 atol=tolerance
@test est1[1].θ == 1000.0
@test est1[2]≈0.36278306128485405 atol=tolerance
...
@test est[1].α≈1.9335240463440206 atol=tolerance
@test est[1].θ == 26.0
@test est[2]≈0.03204776085711486 atol=tolerance

est = estimate_xmin(moby_data, con_powerlaw, xmins = [2,3,4,10,20])
@test est[1].α≈1.9514282451732778 atol=tolerance
@test est[1].θ == 20.0
@test est[2]≈0.04416094210009813 atol=tolerance

est = estimate_xmin(collect(1:100),con_powerlaw)
@test est[1].α≈9.178824791215408 atol=tolerance
@test est[1].θ == 79.0
@test est[2]≈0.1824330509645657 atol=tolerance

est1 = estimate_xmin(population,con_powerlaw)
@test est1[1].α≈2.0337071519535046 atol=tolerance
@test est1[1].θ == 96479.0
@test est1[2]≈0.0 atol=tolerance
...
@test cmpd.V_test_stat≈5.7463540130003254 atol=tolerance
@test cmpd.V_p_val≈0.9999999954405856 atol=tolerance
...
@test cmpd.xmin == 26
@test cmpd.sig_level == 0.05
@test cmpd.V_test_stat≈8.710089666437788 atol=tolerance
@test cmpd.V_p_val≈1.0 atol=tolerance
@test cmpd.V_preff_distr == 1
@test cmpd.C_b == 566
@test cmpd.C_p_val≈0.0 atol=tolerance
@test cmpd.C_preff_distr == 1
...
@test cmpd.xmin == 7.0
@test cmpd.sig_level == 0.05
@test cmpd.V_test_stat≈4.448486373104684 atol=tolerance
@test cmpd.V_p_val≈0.9999956761232807 atol=tolerance
@test cmpd.V_preff_distr == 1
@test cmpd.C_b == 2757
@test cmpd.C_p_val≈0.0 atol=tolerance
@test cmpd.C_preff_distr == 1
